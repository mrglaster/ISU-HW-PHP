...
function generateRandomDate($month=1, $maxday=30, $year=2009){
        if ($month <10) $strval="0".strval($month);
    else $strval=strval($month);
...
function fillJournal($max_records=1000){
        ...
        for ($i=$rowCount+1; $i<$maxIter;$i++){
                $sql = "insert into  calls (id, phone_init, phone_receiver, duration, date) values  (".strval($i).",".'"'.strval(randomNumberSequence()).'"'.','.'"'.strval(randomNumberSequence()).'"'.','.'"'.strval(random_int(20, 5000))>
